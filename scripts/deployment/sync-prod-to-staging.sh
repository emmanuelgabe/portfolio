...
echo "[SUCCESS] Containers found and running"

echo ""
echo "[WARN] This operation will replace all staging data with production data."
...
echo ""
echo "[STEP] Step 1/4: Exporting production database..."
docker exec -t "$PROD_CONTAINER" pg_dump -U "$DB_USER" -d "$PROD_DB" > "$BACKUP_FILE"
echo "[SUCCESS] Export completed: $BACKUP_FILE ($(du -h "$BACKUP_FILE" | cut -f1))"

echo ""
echo "[STEP] Step 2/4: Closing connections to staging..."
...
echo ""
echo "[STEP] Step 3/4: Dropping and recreating staging database..."
...
echo "[SUCCESS] Staging database recreated"

echo ""
echo "[STEP] Step 4/4: Restoring data to staging..."
...
echo ""
echo "========================================="
echo "  [SUCCESS] SYNCHRONIZATION SUCCESSFUL"
echo "========================================="
...
echo "[INFO] Staging DB accessible on: localhost:5434"
echo ""
