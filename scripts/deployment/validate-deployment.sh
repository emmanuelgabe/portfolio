...
case $ENV in
  ...
  prod)
    ...
    ;;
  *)
    echo "[ERROR] Invalid environment: $ENV (local|staging|prod)"
    exit 1
    ;;
esac
...
echo "[SUCCESS] Docker Compose files present"
...
if [ -z "$DB_USER_PASSWORD" ]; then
  echo "[WARN] DB_USER_PASSWORD not set, using 'test' for validation"
...
if [ $ATTEMPT -eq $MAX_ATTEMPTS ]; then
  echo "[ERROR] Timeout: containers did not become healthy"
  echo ""
...
if [ "$FRONTEND_STATUS" = "200" ]; then
  echo "    [SUCCESS] Frontend health OK (HTTP $FRONTEND_STATUS)"
else
  echo "    [ERROR] Frontend health FAILED (HTTP $FRONTEND_STATUS)"
  exit 1
fi
...
if [ "$BACKEND_STATUS" = "200" ]; then
  echo "    [SUCCESS] Backend health OK (HTTP $BACKEND_STATUS)"
else
  echo "    [ERROR] Backend health FAILED (HTTP $BACKEND_STATUS)"
  exit 1
fi
...
if echo "$DB_TEST" | grep -q "1 row"; then
  echo "    [SUCCESS] Database connectivity OK"
else
...
if [ "$STACK_STATUS" = "200" ] || [ "$STACK_STATUS" = "304" ]; then
  echo "    [SUCCESS] Full stack OK (HTTP $STACK_STATUS)"
else
  echo "    [ERROR] Full stack FAILED (HTTP $STACK_STATUS)"
  exit 1
fi
...
echo ""
echo "========================================="
echo "[SUCCESS] Validation successful for $ENV!"
echo "========================================="
echo ""
